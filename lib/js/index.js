...
$(".banerGetFancy").on("click",".topReferencesIsSet",function(){
  if(contactIsSet){                                             
...
$(".banerGetFancy").on("click",".contactIsSet",function(){
  if(!contactIsSet){
    ...
    if($(".lang_help").html() == "sk"){
      $(".getFancyButtonBox").animate({
        ...
        $(".getFancyButton").fadeIn().html($(".form_top_references_help").html());  
      });
    }else{
      $(".getFancyButtonBox").animate({
        width: '-=15px'
