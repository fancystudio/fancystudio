...
$(document).ready(function() {
  ...
  var $container = $('.referencies .row').imagesLoaded( function() {
	  $container.isotope({
		  filter: '*',
		  animationOptions: {
            duration: 750,
	  ...
	  $(".referencies").css("height",$container.css("height"));
    ...
    if(!contactIsSet){
      ...
      if($(".lang_help").html() == "sk"){ 
        $(".getFancyButtonBox").animate({
          ...
          $(".getFancyButton").fadeIn().html($(".form_back_reference_help").html());  
        });
      }else{
        $(".getFancyButtonBox").animate({
            width: '-=19px'
